...
int main( int argc, char *argv[] ) {
	...
	int     miId, numProcs;
	double  *vectorInicial, *vectorLocal, *vectorSumas;
	...
	if(miId == 0) { // ... (G)
		vectorSumas = ( double * ) malloc( numProcs * sizeof( double ) );
	}
	MPI_Gather(&sumaLocal, 1, MPI_DOUBLE, vectorSumas, 1, MPI_DOUBLE, 0, MPI_COMM_WORLD);
	if(miId == 0) {
		sumaFinal = 0;
		for(i = 0; i < numProcs; i++) {
			sumaFinal += vectorSumas[i];
		}
	}
